...
bulk_data <- read_excel("BrkThAI_Mouse In Vitro RNA seq_FPKM Read Counts.xlsx", sheet = 6)
bulk_data <- as.data.frame(bulk_data)

HL017_FPKM_1 <- log1p(bulk_data$`HL017 Replicate 1`)
mean(HL017_FPKM_1)
sd(HL017_FPKM_1)

HL017_FPKM_2 <- log1p(bulk_data$`HL017 Replicate 2`)
mean(HL017_FPKM_2)
sd(HL017_FPKM_2)

gene_means <- apply(cbind(HL017_FPKM_1, HL017_FPKM_2), 1, mean)
gene_sds <- apply(cbind(HL017_FPKM_1, HL017_FPKM_2), 1, sd)

n_genes <- length(HL017_FPKM_1)
n_cells <- 1000
standard_deviation <- 0.3

# Initialize an empty matrix to store the simulated counts
simulated_FPKM_HL017 <- matrix(0, nrow = n_genes, ncol = n_cells)

# Loop over each gene to simulate its counts across 1000 cells
for (i in 1:n_genes) {
  # Generate random normal data for each gene
  simulated_FPKM_HL017[i, ] <- rnorm(n_cells, mean = gene_means[i], sd = standard_deviation)

  # Ensure no negative counts (since gene expression counts cannot be negative)
  simulated_FPKM_HL017[i, simulated_FPKM_HL017[i, ] < 0] <- 0
}

#name the rows and columns to match gene IDs and cell numbers
rownames(simulated_FPKM_HL017) <- bulk_data$GeneID
colnames(simulated_FPKM_HL017) <- paste0("Cell", 1:n_cells)

row_means <- rowMeans(simulated_FPKM_HL017)
head(row_means)

# Check the dimensions
dim(simulated_FPKM_HL017)

Seurat_Object <- CreateSeuratObject(counts = simulated_FPKM_HL017)
